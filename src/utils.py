...
def eval_model_dataloader(encoder_bert, classifier, dataloader, device, detailed=False,
                          criterion=nn.CrossEntropyLoss(reduction='sum'), use_pooled_output=False,
                          verbose = False): #sum
    """
    ...
    if detailed:
        ...
        tea_seqfeat_maps = [[] for _ in range(128)]
        predictions = []
        pooled_feat_maps = []
    ...
    for _, batch in enumerate(tqdm(dataloader, desc="Iteration")):
        ...
        with torch.no_grad():
            if classifier is None:
                ...
                if use_pooled_output:
                    _, pooled_feat = feat # [CLS]
                    preds = classifier(pooled_feat)
                else:
                    sequence_feat, pooled_feat = feat #seq[128,32,768]
                    # mean_sequence_feat = sequence_feat.mean(1)
                    # preds = classifier(mean_sequence_feat)
                    preds = classifier(pooled_feat)
        loss += criterion(preds, label_ids).sum().item() #reduction='sum'
        ...
        if detailed:
            ...
            # sl = input_ids.shape[1] # sequence_length
            # tea_seq_feat = [[] for _ in range(sl)]#错了啊，写到循环里面了可还行
            need_reshape = bs != pooled_feat.shape[0]
            if classifier is None:
                raise ValueError('without classifier, feature cannot be calculated')
            # trans_sequence_feat = sequence_feat.transpose(0,1).contiguous().view(sl,bs,-1)
            # print(trans_sequence_feat.shape) #torch.Size([128, 32, 768])

            for fm, f in zip(tea_seqfeat_maps, sequence_feat):
                if need_reshape:
                    fm.append(f.contiguous().view(bs,-1).detach().cpu().numpy())
                else:
                    fm.append(f.detach().cpu().numpy())
            # print('********************###################################')
            # print(np.array(tea_seq_feat).shape)#(128, 1, 32, 768)
    ...
    # print('********************#tea_seqfeat_mapsshappe')
    # print(np.array(tea_seqfeat_maps).shape)
    ...
    if detailed:
        ...
        tea_seq_f = [np.concatenate(t) for t in tea_seqfeat_maps] if len(tea_seqfeat_maps[0]) > 0 else None
        if n_layer == 24:
            return {'loss': loss,
                    'acc': acc,
...
def eval_model_dataloader_nli(task_name, eval_label_ids, encoder_bert, classifier, dataloader, kd_model, num_labels,
                              device, weights=None, layer_idx=None, output_mode='classification',use_pooled_output=False):
    encoder_bert.eval()
    classifier.eval()
    ...
    for input_ids, input_mask, segment_ids, label_ids in dataloader:
        ...
        with torch.no_grad():
            # sequence_output, pooled_output = encoder_bert(input_ids, segment_ids, input_mask)
            _, pooled_output = encoder_bert(input_ids, segment_ids, input_mask)
            if kd_model == 'kd':
                logits = classifier(pooled_output)
                # if use_pooled_output:
                    # logits = classifier(pooled_output)
                # else:
                    # mean_sequence_output = sequence_output.mean(1)
                    # logits = classifier(mean_sequence_output)
            else:
